...
RUN CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o /out/freesms ./cmd/freesms
...
#
#     mkdir /var/lib/freesms/attachments/b6: permission denied
#
...
COPY --from=build /out/freesms /freesms
COPY --from=build --chown=nonroot:nonroot /out/attachments /var/lib/freesms/attachments
...
# a photograph exists.
VOLUME ["/var/lib/freesms/attachments"]
